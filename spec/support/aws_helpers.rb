...
module AwsHelpers
  ...
  def assume_role
    @role_arn = ENV['SETTINGS__AWS__ROLE_ARN']

    raise 'You must set SETTINGS__AWS__ROLE_ARN' if @role_arn.nil? || @role_arn.empty?
  ...
  def get_bucket
    bucket = ENV['SETTINGS__AWS__S3_BUCKET']

    raise 'You must set SETTINGS__AWS__S3_BUCKET' if bucket.nil? || bucket.empty?
  ...
  def delete_file_from_s3(client, bucket, key)
    # will look like this, role needs additional permissions added to it first though
