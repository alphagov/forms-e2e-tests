...
feature 'Full lifecycle of a form', type: :feature do
  ...
  def build_a_new_form
    ...
    fill_in 'Enter some information to tell people what will happen next',
            with: "We'll send you an email to let you know the outcome. You'll usually get a response within 10 working days."
  ...
  def add_form_submission_email
    ...
    if page.has_content? 'Enter the email address confirmation code'
      ...
      fill_in 'What email address should completed forms be sent to?',
              with: 'govuk-forms-automation-tests@digital.cabinet-office.gov.uk', fill_options: { clear: :backspace }
      click_button 'Save and continue'
      ...
      fill_in 'What email address should completed forms be sent to?',
              with: 'govuk-forms-automation-tests@digital.cabinet-office.gov.uk'
      click_button 'Save and continue'
    end
  end
  ...
  def delete_form
    ...
    return unless page.has_link?(form_name)
    ...
    return unless page.has_css?('.govuk-table')
  ...
  def sign_in
    ...
    index = if using_auth0_passwordless_connection
              '/'
            else
              '/?auth=e2e'
            end
  ...
  def sign_in_to_auth0(using_auth0_passwordless_connection)
    ...
    if using_auth0_passwordless_connection
      ...
      auth0_gmail_password  = ENV.fetch('AUTH0_GOOGLE_APP_PASSWORD') do
        raise 'You must set AUTH0_GOOGLE_APP_PASSWORD to use Auth0'
      end
